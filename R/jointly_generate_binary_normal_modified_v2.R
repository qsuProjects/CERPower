...
BN.rBound = function(p) {
  cat("Finding BN correlation bound\n")
...
mod.jointly.generate.binary.normal=function(no.rows, no.bin, no.nor,
                                            prop.vec.bin, mean.vec.nor,
                                            var.nor, corr.vec, adjust.corrs = TRUE,
                                            zero = zero, one = one){
  cat("Jointly generating binary normals\n")
  ...
  cat("Algorithm will be using the closest positive definite matrix!\n")}
  ...
  if(is.positive.definite(sigma_new)==FALSE) {
    ...
    cat("Final correlation matrix is not positive definite,\n")
    cat("Algorithm will be using the closest positive definite matrix!\n")
  }
