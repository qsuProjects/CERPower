...
override_drug_probs = function(mus0, n.Drugs, n.OtherBins, zero=0.0001) {
  ...
  cat("Overriding drug probabilities\n")
...
upper_tri_vec = function(m) {
  # cat("Upper triangular vectoring\n")
...
add_one_categorical = function(d, n, obs, cat.parameters) {
  ...
  cat("Adding a categorical variable\n")
...
override_static = function(.static.var.name, .id.var.name="id", .d, .obs) {
  cat("Overriding static variables\n")
...
expand_subjects = function(mus3, n.OtherNorms, n.OtherBins, n.Drugs, wcor, obs, zero=0.0001,
                           ...
                           .set_number = NULL, .log_path = NULL) {
  
  cat("Expanding subjects\n")
  ...
  for (s in 1:n) {
    ...
    if(!is.null(.set_number) & !is.null(.log_path) & ( ( ( s - 1 ) %% 500 ) == 0 ) ) {
      cat(paste0(Sys.time(),
                 " Expanding subject ", s, " for ", .set_number,
                 "\n"), 
          file = .log_path, append = TRUE)
    }
    
    cat(paste0("Expanding subject ", s, "\n"))
...
make_one_dataset = function(n, obs, parameters, n.Drugs, pcor, wcor, cat.parameters, zero=0.0001,
                            .set_number = NULL, .log_path = NULL) {
  cat("Making one dataset\n")
  ...
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Generating subject-specific mus for ", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
  ...
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Overriding drug probs for ", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
  ...
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Expanding ever user for ", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
  ...
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Expanding subjects for ", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
  
  d1 = expand_subjects(mus3, n.OtherNorms, n.OtherBins, n.Drugs, wcor, obs, parameters = parameters,
                       .set_number = .set_number, .log_path = .log_path)
...
# everUserExp = expand_matrix(everUser, obs)
# .obs <- 5
# .matrix <- matrix(rbinom(9, size = 1, prob = 0.5), nrow = 3)
...
expand_matrix = function(.matrix, .obs) {
  ...
  # matrix(rep(.matrix[1,],.obs), nrow = .obs, byrow = TRUE)
...
dataset_performance = function(sim, n, obs, n.Drugs, n.OtherBins, n.OtherNorms,
  ...
  cat("Evaluating dataset performance\n")
...
mean_performance = function(results) {
  cat("Computing mean performance stats\n")
...
# n <- .n_subjects
complete_parameters = function(parameters, n, .set_number = NULL, .log_path = NULL) {
  
  cat("Completing parameters\n")
  
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Compiling SDs for ", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
  # # calculate SDs for proportions based on sample size
  # if(!is.null(.set_number) & !is.null(.log_path)) {
  #   cat(paste0(parameters$across.SD[parameters$type=="normal.drug"], collapse = "\t"), "\n",
  #       file = .log_path, append = TRUE)
  # }
  # 
  # if(!is.null(.set_number) & !is.null(.log_path)) {
  #   cat(paste0(parameters$across.mean[parameters$type=="normal.drug"], collapse = "\t"), "\n",
  #       file = .log_path, append = TRUE)
  # }
  # if(!is.null(.set_number) & !is.null(.log_path)) {
  #   cat(paste0(sqrt( parameters$across.mean[parameters$type=="normal.drug"] *(1-parameters$across.mean[parameters$type=="normal.drug"]) ), collapse = "\t"), "\n",
  #       file = .log_path, append = TRUE)
  # }
  ...
  # if(!is.null(.set_number) & !is.null(.log_path)) {
  #   cat(paste0(parameters$across.var, collapse = "\t"), "\n",
  #       file = .log_path, append = TRUE)
  # }
  # 
  if(!is.null(.set_number) & !is.null(.log_path)) {
    cat(paste0(Sys.time(),
               " Setting variances for", .set_number,
               "\n"), 
        file = .log_path, append = TRUE)
  }
...
add_time_function_vars = function(d4, obs, parameters) {
  ...
  cat("Adding time function variables\n")
